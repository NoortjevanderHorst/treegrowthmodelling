...
namespace easy3d {
    ...
    ImGuiContext* ViewerF::context_ = nullptr;
    ...
    void ViewerF::reload_font(int font_size)
	{
		ImGuiIO& io = ImGui::GetIO();
		io.Fonts->Clear();
    ...
    void ViewerF::post_resize(int w, int h) {
    ...
    bool ViewerF::callback_event_cursor_pos(double x, double y) {
    ...
    bool ViewerF::callback_event_mouse_button(int button, int action, int modifiers) {
		if (ImGui::GetIO().WantCaptureMouse)
			return true;
		else
    ...
    bool ViewerF::callback_event_keyboard(int key, int action, int modifiers) {
    ...
    bool ViewerF::callback_event_character(unsigned int codepoint) {
		if (ImGui::GetIO().WantCaptureKeyboard)
			return true;
		else
    ...
    bool ViewerF::callback_event_scroll(double dx, double dy) {
		if (ImGui::GetIO().WantCaptureMouse)
			return true;
		else
    ...
    void ViewerF::draw_menu_file() {
        if (ImGui::BeginMenu("File"))
        {
            if (ImGui::MenuItem("Open", "Ctrl+O"))
                open();
            if (ImGui::MenuItem("Open correspondence", nullptr))
                open_correspondence();
            if (ImGui::MenuItem("Open ts graphs", nullptr))
                open_ts_graphs();
            if (ImGui::MenuItem("Save As...", "Ctrl+S"))
                save();
            if (ImGui::MenuItem("Save current interpolation"))
                save_interpolation();
            ImGui::Separator();
    ...
    void ViewerF::draw_menu_processing() {
        if (ImGui::BeginMenu("Processing"))
        {
            if (ImGui::MenuItem("Correspondence", nullptr))
    ...
    void ViewerF::draw_overlay(bool* visible)
    {
        ImGui::SetNextWindowSize(ImVec2(400 * widget_scaling(), 200 * widget_scaling()), ImGuiCond_FirstUseEver);
